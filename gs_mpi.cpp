...
struct Params {
    int Nx=256, Ny=256;      // global interior sizes
    ...
    int quiet=0;             // 1 => suppress periodic prints
    int niters=-1;           // >0 => fixed iterations (benchmark mode)
};
...
int main(int argc,char** argv){
   ...
   // Put this at file scope (or outside the lambda so you can reset it each iter)
...
auto sweep_color = [&](int color){
    ...
    // 7) Residual L2 over interior (since f=0, r = Δ_h u)
    auto resid_local = [&](){
        double s=0.0;
        for(int j=1;j<=p.Ny;++j){
            for(int i=1;i<=nxloc;++i){
